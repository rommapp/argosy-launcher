...
const std::string ShaderManager::crtShaderFragment =
    ...
    "\n"
    "  HIGHP vec2 fractPart = screenCoords - floor(screenCoords);\n"
    "  fractPart = clamp(fractPart, 0.0001, 0.9999);\n"
    "  HIGHP vec2 scanlineCoords = fractPart * 2.0 - vec2(1.0);\n"
    "\n"
    "  lowp float mask = 1.0 - abs(scanlineCoords.y);\n"
    "\n"
...
const std::string ShaderManager::lcdShaderFragment =
    ...
    "\n"
    "void main() {\n"
    "  HIGHP vec2 floorPart = floor(screenCoords + 0.5) - 0.5;\n"
    "  HIGHP vec2 fractPart = screenCoords - floorPart;\n"
    "  fractPart = clamp(fractPart, 0.0001, 0.9999);\n"
    "\n"
    "  mediump vec2 threshold = vec2(1.0 / screenDensity);\n"
    "  mediump vec2 x = fractPart;\n"
    "  x = 0.5 * (smoothstep(vec2(0.0), threshold, x) + smoothstep(vec2(1.0) - threshold, vec2(1.0), x));\n"
    "  mediump vec2 sharpCoords = (floorPart + x) / textureSize;\n"
    "\n"
    "  lowp vec3 texel = texture2D(texture, sharpCoords).rgb;\n"
    "  lowp vec3 pixelHigh = ((1.0 + BRIGHTBOOST) - (0.2 * texel)) * texel;\n"
    "  lowp vec3 pixelLow  = ((1.0 - INTENSITY) + (0.1 * texel)) * texel;\n"
    "\n"
    "  HIGHP vec2 gridCoords = fractPart * 2.0 - vec2(1.0);\n"
    "  gridCoords = gridCoords * gridCoords;\n"
    "\n"
    "  lowp float mask = 1.0 - gridCoords.x - gridCoords.y;\n"
    "\n"
    "  gl_FragColor = vec4(mix(texel, mix(pixelLow, pixelHigh, mask), screenMaskStrength), 1.0);\n"
    "}\n";
